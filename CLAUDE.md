...
| **The 3D board migration** (anything that renders the board, stores voxel state, or retires a 2D-board rule) | [`PROMPTS/PLANNING/RENDER3D_MASTER_PLAN.md`](PROMPTS/PLANNING/RENDER3D_MASTER_PLAN.md) | 🟡 v1.19 — **`RENDER3D` is the DEFAULT since 2026-09-19 (`RENDER3D=0` = the 2D board, kept only as a comparison until R3D-END; the selftest harness pins it until R3D-8).** **Director, 2026-09-23: the 2D board retires when NOTHING depends on it, not when the look matches — the look is not a gate.** R3D-8 was renamed **R3D-END**, and the road to it is six sequential stages: **R3D-8** the safety net on the 3D path (selftests on the 3D board, a 3D pixel gate earned, the 21-voxel persistence loss fixed, the web export, the 2D reference set archived once); **R3D-9** the 3D board reads nothing of the 2D renderer (look constants, crack records, piles); **R3D-10** the simulation writes no tiles (the cook's PACKAGE still resolves 2D tiles); **R3D-11** gameplay off `floor_layer` (→ `GroundGrid`); **R3D-12** the load builds no 2D board (~20.3 s of 2D bakes on the Moto's 3D boot); **R3D-13** one path (flags, tools, spikes) and the last Moto + Galaxy baseline; then **R3D-END** (delete + canon, the Director ratifies). After the end: R3D-LOOK (the look register, 9/10 bar), R3D-WORLD (screen-space models to world space), R3D-ROT (rotation, was R3D-9), R3D-LIGHT, R3D-CLAIMS, R3D-BUFFER, R3D-GLB. Floors and slabs show the whole facade (Director, 2026-09-23). **Built (R3D-0 to R3D-7):** the packed `VoxelStore` (one authority), the board (D26 camera, merged faces, per-cell light/soot planes, threaded remesh), actors and props as billboards, world-space VFX, ground overlays and fog (`GroundCanvas3D`), picking (`pick_cell`, `GroundGrid`), glass, decals, dents, the `OcclusionSet`-driven cutaway, roofs as an entity, the guard reveal silhouette (`GUARD_REVEAL`, off). ⚠️ **Identity gates: `python3 tools/persistent/board_probe.py gate` (voxels + cell planes, never a tile; `--env` flips a flag), `shot_3d_gate.py` (a real shot reaches the 3D board), `occ_canonical_gate.py` (order-independent occlusion/cutaway digests).** ⚠️ **Under the 3D board, a step that still drives the hidden 2D board is pure waste (overlay refresh 25 ms, `apply_occlusion` 68-291 ms on the Moto): skip it.** The Moto g04s is the constraint (the Galaxy A16 is 3-4x faster). **Rules: a NEW VFX stores world-space state (ground + height), never screen pixels; a level is asked of `ground_plane_level()`, never a literal.** The legacy audit and the open register are at the top of the plan. |
