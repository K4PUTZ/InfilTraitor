...
| **Prediction / simulate-without-committing** (any preview, estimate, or "what if") | [`PROMPTS/PLANNING/PREDICTION_MASTER_PLAN.md`](PROMPTS/PLANNING/PREDICTION_MASTER_PLAN.md) | 🔵 planned, no code. Pure `simulate(action) -> WorldDelta` + `commit()` + cache. §2 is the authoritative mutation inventory: the ONLY Voxel writers in the blast pipeline are `apply_container_damage()`/`apply_crater_damage()` (7 `set_damage()` sites); the soot layer is already pure; firearms use `apply_point_impact()` and share neither |
