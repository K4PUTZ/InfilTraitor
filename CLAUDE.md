...
| **Draw order / depth on the board** (anything about what covers what on screen) | [`PROMPTS/PLANNING/RENDER_ORDER_MASTER_PLAN.md`](PROMPTS/PLANNING/RENDER_ORDER_MASTER_PLAN.md) | 🔵 v1.0 DESIGNED, SPIKE-GATED 2026-09-10, nothing built. **`z_index` in this project encodes HEIGHT (`WALL_BASE_Z_INDEX + relative_level`), and depth is independent of it** (`OCCLUSION` `O5`) — three systems have now paid for that: props hand-roll a depth solver, the occlusion wireframe uses per-level panels, and a glass pane composites over the wall in front of it. **Y-sorting has never been enabled anywhere in this project** — `y_sort_origin` is set in three places and does nothing without `y_sort_enabled`. The plan keeps the per-level `z` scheme intact on purpose (that is what protects OCC-23, OCC-03 and the twelve overhead overlays) and resolves depth inside a level band. Task 1 is a spike; the Director-authorised fallback is glass-on-top + a map-authoring warning. ⛔ `GLASS` §19 and its dependency on `OCCLUSION` §7 were rejected 2026-09-10 — do not build from either |
