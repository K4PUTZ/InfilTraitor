...
| **The 3D board migration** (anything that renders the board, stores voxel state, or retires a 2D-board rule) | [`PROMPTS/PLANNING/RENDER3D_MASTER_PLAN.md`](PROMPTS/PLANNING/RENDER3D_MASTER_PLAN.md) | 🟡 v1.8 — **`RENDER3D` is the DEFAULT since 2026-09-19 (`RENDER3D=0` = the 2D board; the selftest harness pins it).** **R3D-0 to R3D-5 built (2026-09-18): the board, the actors and props (billboards), every in-world VFX (`ParticleMath`, depth-tested `MultiMesh` fields), the ground overlays and the fog (`GroundCanvas3D`) and picking (`Board3DLive.pick_cell`, `GroundGrid`) all live in the 3D world. R3D-6 (look parity): glass ratified, decals and dents built, facades/embers/floor shards deferred as look tuning. R3D-7 (cutaway, driven by the 2D `OcclusionSet`: solid 2-voxel base, dithered ghost above, white outline with dashed far edges) built and approved; next: wall picking (Director's call), then R3D-8.** The register of what is still open is at the top of the plan. **Rules set there: a NEW VFX stores world-space state (ground + height), never screen pixels; a level is asked of `ground_plane_level()`, never a literal.** Direction ratified by the Director 2026-09-15. Voxels stay the unit of simulation but become a **packed store** — one authority. A `Voxel` object costs ~925 B on the Moto (~191 MB on PLAYGROUND; 316 MB on a desktop debug build). **Every stage's identity gate is `python3 tools/persistent/board_probe.py gate`** (`--env` flips the stage's flag); it reads the voxel containers and cell planes, never a tile. **Layout B is confirmed; the packed `VoxelStore` runs in shadow behind `VOXEL_STORE=1`, and `board_probe.py shadow` requires it to equal the objects per voxel** — every R3D-1c reader flip keeps that gate green The board is drawn by Godot 3D: D26 camera, merged faces, per-cell planes. The 2D board and its canon retire at **R3D-8** only. Every stage runs shadow → flip → delete, with `BoardProbe` identity gates and a Moto A/B. Zones materialised on damage were considered and not taken (§0.2) |
