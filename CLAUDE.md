...
| **The 3D board migration** (anything that renders the board, stores voxel state, or retires a 2D-board rule) | [`PROMPTS/PLANNING/RENDER3D_MASTER_PLAN.md`](PROMPTS/PLANNING/RENDER3D_MASTER_PLAN.md) | 🟡 v1.15 — **`RENDER3D` is the DEFAULT since 2026-09-19 (`RENDER3D=0` = the 2D board; the selftest harness pins it).** **R3D-0 to R3D-5 built (2026-09-18): the board, the actors and props (billboards), every in-world VFX (`ParticleMath`, depth-tested `MultiMesh` fields), the ground overlays and the fog (`GroundCanvas3D`) and picking (`Board3DLive.pick_cell`, `GroundGrid`) all live in the 3D world. R3D-6 (look parity): glass ratified, decals, dents and the rim wedge built and MEASURED on the Moto (a few percent of frame time, no memory), facades/embers/floor shards deferred as look tuning. R3D-7 (cutaway, driven by the `OcclusionSet`) is CLOSED (2026-09-20): cost cut on the Moto, revealed actors behind walls (gameplay-driven striped silhouette, `GUARD_REVEAL`, off by default), and ROOFS AS AN ENTITY (`roofs` map section) opened by adjacency of slabs and stored per GU. Decals and shots on materials TESTED (2026-09-21): a real shot never reached the 3D board, fixed and gated by `python3 tools/persistent/shot_3d_gate.py`; the impact smoke/sparks now anchor in front of the struck face and `MUZZLE_LEVEL` is derived from the ground plane (both closed 2026-09-21). The shot path, the sparks, `GUARD_REVEAL` with 8 guards and tap/select/walk/pan are TESTED on the Moto (v1.11); the pinch, the Galaxy shot path, the reap hardening, a second map and the unknown-roof-kind case are closed (v1.12): nothing R3D-7 is open. Blast soot now lands after the crater in timed steps. **Director, 2026-09-21: the fine look adjustment is NOT deferred any more, and "identical" means the SYSTEM (a tone/hue difference is fine; the bar is 9 out of 10 closeness to the 2D feature, graded by the Director). Order: grade the register (v1.14/v1.15), fix system defects first (FIRE, EMBERS, the end-of-blast light jump), then the items graded below 9, then R3D-8 as the LAST stage (irreversible, needs ratification).** ⚠️ **A change to how the occlusion set or the cutaway geometry is built is judged by `python3 tools/persistent/occ_canonical_gate.py` (order-independent digests), not by hashing emission order.** ⚠️ **Under the 3D board, a step that still drives the hidden 2D board is pure waste (overlay refresh 25 ms, `apply_occlusion` 68-291 ms on the Moto): skip it behind `draws_cutaway()`.** The Moto g04s is the constraint (the Galaxy A16 is 3-4x faster here). The register of what is still open is at the top of the plan. **Rules set there: a NEW VFX stores world-space state (ground + height), never screen pixels; a level is asked of `ground_plane_level()`, never a literal.** Direction ratified by the Director 2026-09-15. Voxels stay the unit of simulation but become a **packed store** — one authority. A `Voxel` object costs ~925 B on the Moto (~191 MB on PLAYGROUND; 316 MB on a desktop debug build). **Every stage's identity gate is `python3 tools/persistent/board_probe.py gate`** (`--env` flips the stage's flag); it reads the voxel containers and cell planes, never a tile. **Layout B is confirmed; the packed `VoxelStore` runs in shadow behind `VOXEL_STORE=1`, and `board_probe.py shadow` requires it to equal the objects per voxel** — every R3D-1c reader flip keeps that gate green The board is drawn by Godot 3D: D26 camera, merged faces, per-cell planes. The 2D board and its canon retire at **R3D-8** only. Every stage runs shadow → flip → delete, with `BoardProbe` identity gates and a Moto A/B. Zones materialised on damage were considered and not taken (§0.2) |
| **Device measurement** (Android handsets, memory, frame budget, `DevFlags`, telemetry, scenarios) | [`PROMPTS/PLANNING/DEVICE_DIAGNOSTICS_MASTER_PLAN.md`](PROMPTS/PLANNING/DEVICE_DIAGNOSTICS_MASTER_PLAN.md) | 🟢 v1.8. Budget: 30 fps / 33.3 ms on playback frames (§0.5), on the Moto g04s and the Galaxy A16. The chain is `export_android.py` → `device_run.py --mem-poll` → `bench_analyze.py`. A flag reaches the APK only through `DevFlags` (`adb push` of `dev_flags.cfg`). §15 holds the 2D-vs-3D evidence |
