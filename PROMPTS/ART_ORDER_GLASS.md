...
### 6.5 §4's wiring — steps 2 and 6 DONE 2026-09-02

- **Step 2, the resolver category.** `TextureResolver._validate_dimensions()`
  now has a `fracture_` branch on the facade's own 64 × 32 contract — the sheet
  rides the facade compositing path verbatim, so a sheet that disagreed with the
  facade page would break G-D21's offset arithmetic.
- **Step 6, the constant and the missing wiring check.**
  `GlassMaterials.FRACTURE_WIDTHS` names the two sheets (there, not at a
  consumer, because there IS no consumer yet), and `check_decal.py` now reads it:
  it fails if the roster disagrees with the gate's own, and fails if the resolver
  has no `fracture_` category — the silent Tier.NONE drop.
- **Step 4, the manifest: a genuine no-op.** `ASSETS/materials/manifest.json` is
  GENERATED by `generate_voxel.py` and mirrors `IMPACT_DECAL_MATERIALS`, which
  step 3 does not change (below). Hand-editing it would create the drift the
  selftest exists to catch.
- **Step 5, the file-count table.** `ART_SPECIFICATIONS.md` §7 is 42 → 45 with a
  `shard` row, and §7b is DELIVERED rather than ORDERED.

⚠️ **Wired is not used.** G-D21's re-anchoring is unbuilt, so nothing requests a
sheet. The gate prints that on its own line precisely so two green wiring lines
are not read as a working feature.

### 6.6 ⚠️ STEP 3 IS NOT AN EDIT — IT IS A CONTRADICTION, AND IT IS LEFT OPEN

§4 step 3 says to add `"glass"` to `IMPACT_DECAL_MATERIALS` and raise
`glass.json`'s `crack_factor` above 0.0, *"the two must move together"*. Read
...
- `crack_factor > 0` demands a crack DECAL family. The order attributes the
  coupling to `voxel_decal_selftest` **[12]**, but [12] actually ties
  `crack_factor` to `IMPACT_CRACK_MATERIALS` **and to
  `decal_crack_glass_{0,1,2}.png` existing on disk** — art G-D21 says must never
  be authored.

So glass is the first material where "this material cracks" and "this material
has a crack decal family" come apart, and [12] cannot say that today. The fix is
a decision for the Director, not a patch: either [12] learns that a
...
**Step 7** (fire a pistol and a rifle at the GLASS map's pane) is blocked behind
the same thing: it needs G-D21 built, not wired.
