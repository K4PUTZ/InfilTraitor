...
> the guards render as the rigged mesh itself (D35's source, one joined skinned mesh, animations as actions),
> lit by the board's own cell planes — never by Godot lights. 9 walking rigs cost +1.0 ms, 20 static props
> +1.5 ms (real lamps: +2.4 and +9.7). **This reopens D17, D34, D42, D44 and D62** (each row says how); D26,
> D35, D46, D47, D61 and D63 hold. The build is `RENDER3D` R3D-ACTORS and R3D-PROPS, after R3D-END.
...
- ~~Characters have **no voxel representation at all** today — `agent.gd` and
  ...
  **SUPERSEDED 2026-08-16/17.** Both now render BAKED SPRITES from one rig —
  the agent at three postures, four facings and a 32-phase walk
  (`CHARACTER_MASTER_PLAN` Part 2 §10), the guard in a different palette from the
  same mesh (Part 7). The measured silhouette is **104 × 222 px**, not 44 × 61,
  and `occlusion_set.gd` was re-synced to it. Characters still have no VOXEL
  representation and no connection to the material/damage system — that half of
  the sentence stands, and it is Part 9.
...
| **D52** | **The visual target is a STYLISED figure between Sonic's volumetric cartoon and Moonwalker's human silhouette — and D35's "action figure" is its ARCHITECTURE (swappable parts), not its FINISH (visible joints).** *(Director, 2026-08-15: "o modelo não vai ser bonequinho necessariamente, queremos um pouco mais de detalhes... Um estilo entre o cartoon volumétrico do Sonic e a silhueta humana do Moonwalker. Outros exemplos são Dick Tracy, James Bond, Spy.")* **Refines D35 rather than superseding it**, by splitting a row that had been read as a single thing. D35's *"visible joints, swappable parts, low-poly, not realistic"* holds two separable halves: **swappable parts is construction**, and it is the half that carries D34 — layering reads as native construction instead of as a compositing artifact. **Visible joints is surface finish**, and it is the half the Director is declining. Registering the split has a concrete cost attached, paid inside this session: an argument had been built on visible joints *deleting* the hard part of a procedural hand (the interdigital blend, the thenar mass, the rotated thumb frame), and the corrected reading puts all three back. **The reference set is coherent, and it is silhouette-first** — Dick Tracy, Bond and TF2's Spy all resolve to one shape at distance and carry their detail in *tailoring* rather than in anatomy, which is precisely the property a 196 px token needs. The Moonwalker anchor has been stable since D16 (2026-07-26). **The bar this sets for geometry:** volume and proportion do the work; anatomical fidelity does not. | ✅ Ratified (Director, 2026-08-15) |
| **D53** | **The costume is a REVERSIBLE-COAT DISGUISE RIG — and it gives D37's stealth mode its visual form at the cost of exactly one new socket.** *(Director, 2026-08-15: "Podemos usar esse estilo do Dick Tracy com o sobretudo branco por fora e preto por dentro (da pra inverter conforme a necessidade - gasta AP para fazer a troca). A camisa e armband azul + a gravata branca fazendo uma homenagem ao Michael Jackson, e usando o terno inteiro preto, incluindo o chapéu. De tal forma que, ele pode virar o sobretudo do avesso para ficar preto, e tirar o capuz das costas, por dentro do terno, e colocar sobre a cabeça, ao mesmo tempo em que tira o chapéu e enfia por baixo do sobretudo, numa suposta mochila. Assim fica uma silhueta de balaclava, mas ele pode também voltar com o chapéu e sobretudo no momento conveniente.")* **This is not a new mechanic — it is the missing visual form of one that was designed and mute.** D37 states that the hood is *"the first real consumer of §10.1's tier-1 Civilian clothes — perfect disguise, which has existed in the design with nothing reading it"*, and justifies it with the Director's own *"entrar numa festa de gala pela frente, e de repente ir para outra área da casa."* White coat + fedora **is** the gala; black coat + hood **is** the other area. **What it costs against the existing architecture, itemised because the answer is nearly nothing:** the hood and the overcoat are already `back_upper` layers (D37; hood rigid, coat deforming via D43); the armband already has socket `arm_L`; the white⇄black inversion is **one geometry and a colour swap, which D34 classes as a free shader uniform** — it does not touch §8's multiplicative term; the stowed hat needs no stowed geometry, only a layer switched off; and the AP price fits the ratified 2-AP turn economy, which already prices *"improving cover costs 1 AP"*. **The one real gap, and the only structural addition:** `CHARACTER_MASTER_PLAN` §4.3 had **no head socket** — Part 1's fedora is part of the body mesh — so a removable hat requires `head` (rigid) to be added to that table. **Two numbers deliberately left open** because they are tuning, not architecture: the AP cost of a flip, and whether the coat's visible inner lining needs two-sided material at showcase fidelity (a Beta question under D54). | ✅ Ratified (Director, 2026-08-15) |
| **D54** | **Alpha closes the character's MECHANICS; detail and finish are deferred to Beta.** *(Director, 2026-08-15: "vamos montar a parte técnica agora, finalizar toda a mecânica do personagem até o fim da fase Alpha, e o detalhe/acabamento pode ficar para a segunda fase Beta.")* This is the phase rule the character track had been missing, and it resolves a tension that had been settled ad hoc each time it came up: every previous argument about how good the model needs to look was really an argument about *when*. **It also makes D52's bar operational** — "volume and proportion do the work" is not a compromise accepted under protest, it is the Alpha specification, and the finish that D52 declines is scheduled rather than refused. **Consequence for how work is judged:** during Alpha, a character deliverable is complete when the mechanic reads on screen, and an unfinished surface is not a defect. The inverse also binds — a beautiful model that does not hold a weapon, take cover, or flip its coat is not Alpha progress. | ✅ Ratified (Director, 2026-08-15) |
| **D55** | **D48's ordering is REVERSED: the gameplay figure leads, and the showcase model is deferred — possibly to 2D.** *(Director, 2026-08-15: "O boneco menor é a nossa preocupação principal agora, já que ele é fundamental para o posicionamento, etc. ... Depois vamos detalhar melhor o modelo do menu, podendo inclusive decidir fazer só 2D.")* **The reversal is principled rather than a change of mind, and the reason matters for whether it holds.** D48 put Part 8 first because *"the gameplay figure takes its design from it — building Part 2 first would mean authoring it twice."* The dependency was never on the showcase **model** existing; it was on the **design** being settled. D52 and D53 settle it — style, reference set, costume, and the disguise states — so the dependency is **discharged by another route, not ignored**, and the double-authoring risk D48 was protecting against does not return. **The consequence that matters outside this plan:** Part 2 unblocks, and with it firearm aim mode (`WEAPON_MASTER_PLAN` §5c / D31–D36) and W-PRECOOK, both of which had been waiting on Part 8 at D48's explicitly-stated price. **⚠️ One exposure created and left open on purpose:** D33 splits the two representations as *"gameplay silhouette shows TIER; the big model shows IDENTITY"*, and a 2D showcase changes what that half of the split is. Not decided here; flagged so it is not discovered later as a contradiction. | ✅ Ratified (Director, 2026-08-15) |
| **D56** | **The hand's fidelity bar is "Diablo" — pose-capable, never anatomically correct — and the weapon composites at a RIGID socket rather than being gripped.** *(Director, 2026-08-15: "Precisamos que ele consiga empunhar uma shotgun, uma pistola, etc. Ele não precisa ter a mão perfeita, mas possível de assumir as poses necessárias, como no Diablo.")* **This closes the question that opened the whole line of enquiry** — the Director had raised a realistic hand as the hardest case an algorithm could face (*"se a gente conseguir esculpir ela com algoritmos conseguimos fazer qualquer coisa"*) — and it closes it by **removing the requirement rather than meeting it**. Three facts make the reduced bar exact rather than vague: `hand_R`/`hand_L` are declared **rigid** in §4.3, so no finger deforms around a barrel; **D40 indexes the weapon layer on GRIP (≈3), not on pose**, so idle, walk and turn share one hand configuration; and at the ship size the hand is **≈19 px with ≈2 px fingers** (derived: 1.898 m rendered at 196 px = 103 px/m, against a ~18.5 cm hand). **What must still be true:** the hand must read as *holding* in ~3 configurations across 4 yaws. **What is now explicitly out of scope:** finger articulation, interdigital webbing, and the palm-to-finger blend surfaces — which is the same geometry D52 put back and D56 now declines on separate grounds. | ✅ Ratified (Director, 2026-08-15) |
| **D57** | **Character geometry is ASSEMBLED FROM CC0 PARTS — a sum of small good elements, lightly altered — and CC0 is the licence FILTER, not merely the preference.** *(Director, 2026-08-15: "poderíamos buscar bibliotecas open source, pegar os diversos elementos menores com boa qualidade, mudar um pouquinho pra não ficar tudo muito comum, e gerar um modelo grande, que vira a soma das partes, e não um objeto pré-fabricado e nem uma autoração super complexa.")* **Operationalises D51 by naming what passes the filter and what does not**, researched 2026-08-15 rather than assumed. **Passes (CC0, commercial, no attribution owed):** Quaternius *Modular Character Outfits* (62 separate parts — and the repo already carries two Quaternius packs with `ATTRIBUTION.txt`, so the import convention exists); Blender Studio *Human Base Meshes* (17 meshes, including hands, heads and feet as separate assets); the Sketchfab *Clothing And Character Kit 1.0* and *Military Character Kit 1.1*. **Fails, and both would have been easy to adopt by mistake:** **Meshy's free tier publishes under CC BY 4.0**, not CC0 — commercial use is allowed but carries a permanent attribution obligation attached to the shipped game (its paid tier grants a private licence); and **SMPL-X / MANO**, the standard parametric body and hand models that dominate any search on this topic, are licensed **for non-commercial scientific research only**. **A legal point that inverts the natural intuition and therefore bears stating:** under CC0 there is **no obligation to alter anything** — material could ship verbatim. *"Mudar um pouquinho"* is therefore a **pure art requirement**, which means it is bounded by taste and by nothing else. D51's risk is the opposite direction: non-CC0 material entering unnoticed. **One tool evaluated and declined as unnecessary:** UniRig (Tsinghua/Tripo, SIGGRAPH 2025, open source) auto-rigs arbitrary meshes, but the gameplay figure uses the **verified 20-bone skeleton that already exists** and D48 preserved, and per D55 the showcase model barely animates — so auto-rigging solves a problem this project does not have. | ✅ Ratified (Director, 2026-08-15) |
| **D58** | **The work runs as TWO PARALLEL FRONTS — conceptual art and technical — and the technical front must not presuppose the art front's answer.** *(Director, 2026-08-15: "quero trabalhar mais na parte conceitual da arte primeiro. Vamos organizar isso em duas frentes distintas.")* **Front A (conceptual)** exists to close named decisions with images, not to gather mood: the degree of stylisation, whether joints read at all, silhouette legibility across D44's four facings, the §4.8 palette, the silhouette-class count (`CHARACTER_MASTER_PLAN` §9 #2 — the single number that most moves §8's budget), and the hat's height (the start scene already draws both candidate lines). It inherits one non-negotiable rule from S1, 2026-08-14: **every concept is also checked at 196 px**, because six compression variants were obvious at 8× and indistinguishable at ship size. **Front B (technical)** prepares import, scale and assembly so that execution is mechanical once A lands. **The binding constraint between them:** B is built so the same assembly machine works whether joints are visible or fused — the difference is a boolean/remesh step switched on or off at the end. Building B around either answer would make it hostage to a decision that has not been taken. **Hardware split, recorded because it is load-bearing and not obvious:** Front A runs on the Director's PC (Ryzen 7 5800X3D / RTX 3060 Ti **8 GB** / 16 GB), which is exactly at the floor for the tools that matter — SDXL fits at fp16/1024², Flux fits quantised at GGUF Q4/Q5, TRELLIS 2 needs low-VRAM mode at 512³. Front B runs on the Mac mini M1 (16 GB, Blender 5.2 LTS), which has **no CUDA** — Hunyuan3D's texturing path alone wants ~38 GB and its rasteriser fails *silently* there. Generative 3D therefore lives on the PC as an experiment, never as a pipeline dependency. | ✅ Ratified (Director, 2026-08-15) |
| **D59** | **The near-black suit family, with specular OUT and volume coming from folds and seams — not from a lighting gradient.** *(Director, 2026-08-16: "sem o brilho em volta e nem o reflexo branco que deixa com aparência de plástico. Tecido não tem reflexo duro, somente manchas opacas.")* Applied to suit, trousers, shoe and hat. The reasoning is arithmetic rather than taste: the runtime relight is MULTIPLICATIVE (`lit = albedo × (ambient + light·N·L)`), so darkening the albedo does not move the lit range, it **shrinks** it — a 0.02 albedo compresses everything into roughly 2..9 of 255. Volume therefore cannot come from the value gradient and has to come from GEOMETRY and albedo edges. Bracketed past its own breaking point before ratifying. **D28's outline is declined for this character too**, so both additive levers are out. **A very dark agent is a design ADVANTAGE in an infiltration game**, not a legibility problem to solve. | ✅ Ratified (Director, 2026-08-16) |
| **D60** | **DEV VISION tints the joints yellow, and the joints keep their own material so that it costs one env var.** *(Director, 2026-08-16: "deixamos as juntas amarelas no DEV VISION, pra facilitar o debug.")* Two bakes rather than a shader tint, because the shader has no channel that says *this pixel is a joint*. **The joints were deliberately given the suit's COLOUR while keeping a separate MATERIAL** in the pass before — merging them would have made them unaddressable. **Extended 2026-08-16:** the tint must survive the motion being debugged; a walk with no dev bake switched the joints off for the duration of every step, which reads as a bug in the overlay rather than as a missing asset. Both cycles load independently, so a session that never opens DEV VISION never pays for the second set (D42). | ✅ Ratified & shipped (Director, 2026-08-16) |
| **D61** | **The step is 0.56 s per GU (2.86 m/s), and the pick was INTERIOR to the bracketed range.** *(Director, 2026-08-16, judging blind: "o ritmo é o B mesmo.")* Replaces `agent.gd`'s 0.13 s, which measured **12.3 m/s over a 1.60 m GU — faster than the 100 m world record**; that constant was never a bug, it was tuned for a 44×61 px vector diamond with no legs to contradict it. Bracketed at 130 / 320 / 560 / 950 ms, labels blind, order seeded with the slowest not last, and — the rule the turn test paid for — **judged on a figure with LEGS**, because a slide contains no discrete beat and can only ever produce another monotonic preference. 320 ms was rejected as faster and 950 ms as slower, so the preference is single-peaked and the range did not need re-running wider. **Second half, same row:** `_step_next()`'s per-tile `EASE_IN_OUT` tween is now LINEAR and chained — five GUs used to be five accelerate-decelerate cycles, which with a distance-driven walk stalls the feet inside every tile while the body glides. | ✅ Ratified (Director, 2026-08-16) — closes CHARACTER §9 #12 |
| **D62** | **THE CREATION PIPELINE IS PROVEN. Proportion and viability are closed questions; motion QUALITY is what remains.** *(Director, 2026-08-16: "para a nossa intenção aqui de avaliar proporção e viabilidade da nossa pipeline de criação, já serve esse mecanismo. Então podemos concluir que nosso workflow vai funcionar.")* Rig → posed GLB → Godot bake → relit sprite → the playable agent, gated end to end. **This is the row Part 0 existed to earn.** It does NOT ratify the motion — the same message says *"ainda parece mecânico, falta molejo nas articulações"* — and it does not ratify the SITUATIONAL correctness either: the agent is a stealth infiltrator and the shipped walk is a neutral locomotion cycle. Those two open questions moved out to [`MOVEMENT_MASTER_PLAN`](MOVEMENT_MASTER_PLAN.md), which owns what the figure DOES where `CHARACTER_MASTER_PLAN` owns the figure. | ✅ Ratified (Director, 2026-08-16) |
| **D63** | **A faction is a PALETTE on one mesh — and what separates two figures at ship size is the removal of the agent's high-contrast signatures, not the suit's hue.** *(Director, 2026-08-16: "gerar uma versão do boneco diferente como inimigo, com outra aparência e cores.")* This is D34's "colour is a free shader uniform" cashed on a real second faction: one rig, one mesh, one renderer, a different frame family. **The mechanism worth keeping** is that the agent is near-black by D59, so an enemy that is merely a *different* near-black is the same silhouette in practice; what reads is his white shirt (0.86), white sock (0.90) and red hatband, and the enemy palette removes all three. Measured: identical silhouette (8961 opaque px both), mean RGB (56,55,58) vs (86,90,77) — **49.5 levels apart** — near-white px falling 693 → 243, the remainder being the weapon, which no palette recolours. **⚠️ The COLOURS half only.** The Director asked for *"outra aparência e cores"*; a different hat or silhouette is GEOMETRY, which is the axis D34 says multiplies, and it must be priced rather than assumed cheap. D41's authored head turn is also untouched. | ✅ Ratified — colours shipped 2026-08-16; appearance half open |
| **D64** | **Actors render as LIVE skinned meshes, lit by the board's cell planes — the frame bake retires for gameplay.** *(Director, 2026-09-23: "Aprovado S2", after R3D-SPIKE-3D.)* The source was always a rigged mesh (D35); the bake existed to put it on a 2D board. On the 3D board the mesh itself stands in the scene: one joined skinned mesh per figure (the 62 bake parts joined — all are Armature-modifier bound, so the skin survives), animations as keyed actions (`tools/asset_generation/r3d_live_rig_export.py` keys the very `p3_walk_export` phases), lit by `spike_mesh_planes.gdshader` — the floor cell's light bucket from the board's `cell_plane` array, a fixed key term on the normal, the mesh's albedo, **no Godot light** (12 real lamps cost +7 ms per screenful of mesh on the Moto's Mali). **Measured, Moto g04s, PLAYGROUND, zoom 0.5:** 9 walking rigs +1.0 ms/frame and +2 draw calls; the unjoined rig was +124 draw calls. **What it does to the other rows:** D17 (normal-map relight of sprites) is superseded for gameplay — the mesh has real normals; D34's multiplicative RAM axes lose their reason (one mesh + actions per archetype, colour stays a free uniform per D63); D42's RAM constraint now favours the mesh (RAM against the atlases NOT measured yet); **D44's four facings lose their rendering reason — whether gameplay keeps four facings is a DESIGN question, open**; D62's pipeline becomes rig → GLB with actions (the proof of proportion and viability stands). D26 (the camera), D35, D46, D47 (the GU-boundary snap), D61 (0.56 s per GU) and D63 hold. **Not done:** the weapon in the hand, soot and the cutaway on a mesh, the `AgentSprite` facing/pose/walk bridge, RAM. **No `.blend` holds an action:** the walk exists only as the 32 posed phases the p3 scripts build, so every other motion needs the same keying. | ✅ Ratified (Director, 2026-09-23); build = `RENDER3D` R3D-ACTORS |
| **D65** | **Static props are meshes lit the same way; destructible props are voxels in the store.** *(Director, 2026-09-23: "Aprovado ... S3".)* A pre-rendered prop is right for one camera angle only and R3D-ROT brings rotation back, so a baked billboard would need four views per prop. **Measured, Moto:** 20 single-mesh props (26 080 tris) +1.5 ms with the planes shader (+9.7 ms with real lamps); 20 heavy stand-ins (200 800 tris, 1 280 parts) +4.3 ms — an ASSET budget question (one mesh per prop, a triangle cap), not a renderer one. A prop that can break is a voxel container like a wall (`MATERIALS_MASTER_PLAN` M5), so destruction, soot and light come free. `PropBillboard3D` (the grenade, the collectible) retires with R3D-PROPS. **Not done:** a real prop asset (none exists; the stand-ins were a CC0 rifle and the posed agent statue), the triangle cap's number. | ✅ Ratified (Director, 2026-09-23); build = `RENDER3D` R3D-PROPS |
...
| Agent silhouette on screen (baked sprite, no voxel) | **104×222 px** — measured off the bake 2026-08-16, was 44×61 as a vector | `agent.gd` `SILHOUETTE_WIDTH`/`SILHOUETTE_HEIGHT`, re-synced into `occlusion_set.gd` |
