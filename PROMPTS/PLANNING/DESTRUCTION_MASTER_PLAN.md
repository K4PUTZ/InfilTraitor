...
`VoxelBoard.process_dirty()` rewrites *only the changed cells* → and the TIC
...
| **D17** | **Floor lives in negative storey space; everything else stays exactly where it is.** Walls/blocks/props keep their existing storey-0-and-up placement completely unchanged. Audited and confirmed: the only two places a wall's base storey is decided are `edge_extractor.gd:97` (`"min_storey": 0` for ordinary walls) and `room_builder.gd:611` (per-block authored `storey` value, read as-is) — neither is touched by this decision. Occlusion's base band (`OcclusionSet.BASE_VISIBLE_LEVELS`) is already relative to each edge's own base level, not an absolute 0, so it follows for free. This confines the entire floor-geometry effort to `VoxelBoard`'s own internal bookkeeping: `_voxel_layers` moves from a 0-based `Array[TileMapLayer]` to a `Dictionary[int, TileMapLayer]` keyed by the true (possibly negative) level — GDScript's `array[-1]` means *last element*, not *grow downward*, and `_set_voxel_cell()` previously hard-rejected `level < 0` outright. **Alternative considered and rejected:** shifting every wall/block/prop up a full storey so the floor could occupy storey 0 conventionally — rejected because it touches the tested, working wall/junction/occlusion pipeline across multiple files for no functional gain negative storey doesn't already deliver at lower risk. | ✅ Ratified |
...
(`_ensure_wall_levels()` creates one layer per level, and floors/slabs/ceilings
...
- **Not done yet, deliberately** (next wave, consumes this core): wiring
  `variant_for()` into `VoxelBoard`'s `TileSet` (registering the 8 earth
...
- `voxel_board.gd`'s `MATERIALS` extended with `earth_0`..`earth_7`,
...
- `voxel_board.gd`'s new `register_slab(slab)` — iterates a `Slab`'s voxels,
...
- **Evidence:** `slab_render_selftest.gd`, 8/8 PASS — 64-voxel generation
  ...
  `register_slab()` is idempotent (two calls, identical output); the two-Slab
...
implementation.** `VoxelBoard` now supports both signs:
...
- `_build_voxel_layer_node(level)` extracted as the one formula both
  `_ensure_wall_levels()` (positive) and the new `_ensure_floor_level(level)`
...
- `get_layer(level)` and `_set_voxel_cell(level, ...)` are the two routing
  ...
  positive levels always had). `register_slab()` routes to the correct ensure
...
- **Evidence:** `negative_storey_selftest.gd`, 12/12 PASS — layer creation
  ...
  **D18's lazy contract directly tested**: ensuring level −3 does **not**
  create −1 or −2 along the way; `register_block_levels()` (walls) places all 64 cells
  correctly with negative layers present, `get_layer_count()` stays
  positive-only; `register_slab()` with `slab.level = −1` places 64
...
implementation.** `VoxelBoard.register_fixed_level(gu_cell, level)` —
...
`register_block_levels()` places wall material, just per-level instead of per-storey
...
- **Evidence:** `fixed_floor_selftest.gd`, 5/5 PASS — 64 cells on a fixed
  ...
  (destructible) plus `register_fixed_level()` for levels −8..−2, all 8
...
- `room._slab_registry = SlabRegistry.new()` and `room._voxel_board.clear()`
...
- A new unconditional loop, same `_room_size` coverage as the legacy floor
  ...
  "earth", room._slab_registry)` + `room._voxel_board.register_slab(slab)`
...
eagerly build all 8 levels (`register_fixed_level()` for −8..−2, in
...
`voxel_board.gd`'s `register_slab_solid(slab)`, a sibling to `register_slab()`
...
a randomized palette. `register_block_levels()` (walls) is unchanged and still the
right tool for a block's own body; `register_slab_solid()` is for the
...
`register_slab_solid()` places a real, fixed material with zero per-voxel
...
wood block (via the existing, proven `register_block_levels()`) renders the same
...
- `room_builder.gd::build_from_layout()` iterates `solid_block_instances`
  ...
  exist) and calls `SlabGenerator.generate()` + `register_slab_solid()` for
...
- **Two real gaps found and closed, not assumed away:**
  - `VoxelBoard.process_dirty()` existed (walls only); **nothing consumed
  ...
  - D15's `voxel_destroyed(grid_pos, level, material_id)` signal was
    ...
    `VoxelBoard`, emitted from both erase branches.
...
0. **✅ ANSWERED 2026-07-29 — destruction rewinds with the segment, and commits
   ...
   *(⚠️ **And corrected again 2026-08-27, in the other direction.** The
   2026-08-27 ruling brings a stored soot map back — not as `_base_soot` on the
   Voxel, but as a coordinate-keyed `_soot_map` beside `_base_damage`
   ([`SOOT_STORAGE_REFORM.md`](SOOT_STORAGE_REFORM.md), not started). So this
   paragraph's lifecycle question gets a second payload, and the Director has
   since answered the lifecycle itself: scenario state is **checkpoint-scoped**
   — saved at checkpoints, restored on death, and discarded when the level ends
   or is left. `SaveState.clear_run_state()` is where a new persisted field must
   be forgotten; leaving one behind shows up as the previous level's crater.)*
...
4. **✅ CLOSED 2026-07-30 — D22's DENTED/CRACKED textures render as a
   ...
   - **`Voxel.DamageState.DENTED`/`CRACKED` render through
     `VoxelBoard.damage_variant_material()`**, which maps
     ...
     (`_register_slice`, `process_dirty`, `process_dirty_slabs`'s solid branch)
