...
| **D20** | **The ground bake goes down to level −3, not just the surface.** *(Director, 2026-07-28.)* This **reverses FLOOR-ZONE-BAKE §3b's stated assumption** ("destruction always reveals plain `earth`, never a zone's declared surface"), which was explicitly flagged as an assumption to revisit "once destruction and floor zones are actually seen together in a real played room" — they were, and the exposed generic earth read as a bug. Both `Slab` planes carry the zone material, and the first FIXED level below them (−3) is painted through the same baked page via `VoxelRenderer.set_floor_zone()`, so a two-plane crater never bottoms out on generic ground. Levels −4..−8 stay plain earth: they are only ever seen on the map's outer lateral cut, where reading as raw bedrock is correct. | ✅ Ratified |
