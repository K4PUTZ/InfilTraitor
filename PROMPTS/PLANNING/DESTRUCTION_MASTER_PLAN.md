...
| **D30-CAL** | **CONE's step table stays UNREAD as a distance term, and that is a scope decision with a measured cause.** Wiring `step_multipliers` in as distance for CONE looked canon-correct (D2: *"distance = the step falloff itself"*) and MEASURED as a regression: the shotgun's `[1.0,0.85,0.6,0.35,0.15]` bottoms out at 4 GU, exactly where the bench stands it, so 24 pellets that punched ~7 holes through concrete under the old model produced **zero**. The old CONE path never read that table at all, so switching it on is a behaviour change nobody asked for in this pass. Distance is therefore neutral (1.0) for BOTH shapes today; `ShotPunchTable.cone_distance_multiplier()` is the tested seam firearm range lands on when `WEAPON_MASTER_PLAN` D29's deferred range/dispersion work is taken up. Shotgun `punch` calibrated to 0.24 to restore parity (8 of 24 pellets destroy, vs ~7 before). | ⏸ Open — firearm range curves deferred to D29 |
