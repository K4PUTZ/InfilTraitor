...
## Measuring the real build on a real entry-tier phone — v1.13

> ⏭️ **2026-09-26 — R3D-LIGHT's baseline, DESKTOP (no phone attached; `adb` not on the PATH).** PLAYGROUND, `RNG_SEED=1`, `GRENADE_GUS=25,2;37,2`, `EVENT_FRAMES=1`, `FRAME_PROBE=1`, `REPAINT_PROFILE=1`, scenario `framing portrait; frames 120; zoom 0.75; wait 3; detonate 0; wait 8; detonate 1; wait 8; shoot 0; wait 8; quit`; log local `docs/measurements/desktop_2026-09-26_r3d_light_hitch.log`. **The shot: tail 112.6 ms, repaint 103.9 = occupancy 66.0 · field.build 12.5 · apply 25.1** (stale set, 84 GUs; resolve 4.9, apply+vfx 2.4, render pass 3.4). **A grenade (1st / 2nd):** cook pump 18 / 17 frames (max 50.7 / 50.3 ms), COMMIT frame 32.4 / 32.0 ms (`apply` 33.6 / 15.2 ms, 3 530 / 3 577 cells), CONSEQUENCE frame 62.5 / 43.8 ms, LIGHT beat first frame **68.7 / 68.2 ms** (`light ramp` derive 35.5 / 35.1 ms, 410 / 545 cells moving), detonation mean 16.8 / 16.7 ms (60 fps, vsync-bound), worst 68.7 / 68.2. The map-load repaint (`apply` 1 130 ms) is a load cost, not a hitch. Steady idle: `guard cone smooth draw` 4-7 ms/frame (9 calls) is the largest per-frame item on the desktop. **Use:** proportions only; the Moto ran 3-7x slower on the same frames (R3D-13 tables); the Moto/Galaxy numbers for the R3D-LIGHT gate are still to be taken.
...
> ⏭️ **2026-09-25 — R3D-END END-8, Moto g04s: THE R3D-13 MATRIX ON THE ONLY BOARD (no regression).** Release APK of `ac2eebce` (`export/end8.apk`; the 2D board, `BakeConfig`, `floor_layer` and the atlases deleted), PLAYGROUND, portrait, `RNG_SEED=1`, grenades `25,2;37,2`, shotgun at guard 0, `EVENT_FRAMES=1`, `FRAME_PROBE=1`; scenario `framing portrait; centre agent; zoom 0.75 (12 s); zoom 1.0 (12 s); zoom 0.5 (20 s); detonate 0; detonate 1; shoot 0; wait 8; quit`; two boots, logs local `docs/measurements/device_2026-09-25_moto_end8_{1,2}.log` (git-ignored); the device's `dev_flags.cfg` removed afterwards. **Against the R3D-13 3D row on the same handset (2026-09-24, below):** `[RNG] seeded` -> scenario start **15.5 / 15.2 s vs 16.3**; PSS last poll **1 085 / 1 108 MB vs 1 380** (median 1 037 / 1 050), GL mtrack 271-311 MB, native heap 683-744 MB (vs 736); idle **18.8-19.0 / 18.8 / 19.2-19.4 ms/frame** at zoom 0.75 / 1.0 / 0.5 (gpu 17.2-18.0, draws 76 / 55 / 91) **vs 18.9 / 19.1 / 19.1-21.9** (draws 76 / 55 / 55-91); two detonations (mean / worst frame, COMMIT, CONSEQUENCE, LIGHT ms/f): **28.6 / 30.9 and 28.4 / 31.3 mean, worst 263 / 254 and 256 / 254, COMMIT 238 / 234 and 241 / 235, CONSEQUENCE 236 / 155 and 222 / 178, LIGHT 31.5 / 33.7 and 31.1 / 33.5 vs 29.5-30.2 / 32.4-32.7 mean, worst 319-402 / 693-725, COMMIT 237-239 / 230-233, LIGHT 32-32.4 / 34-35.** So: idle and COMMIT level, memory ~20% down, the second grenade's worst frame 254 vs 693-725 (the CONSEQUENCE tail), both detonations' mean inside the 33.3 ms budget. **The shot after the two grenades (shotgun, pre-cook on): `[AGENT-SHOT-PROF] TAIL TOTAL` 526 / 527 ms** (the Moto's first number for this recipe since the pre-cook was restored; the Galaxy reads ~220 ms, and the Moto read 719 ms on 2026-09-22 before it). The first boot after the install read 37 ms/frame in the first second and 23 ms in the second, then 19 (cold start, as before; the detonation rows of the two boots agree within 3 ms). `bench_analyze.py` printed no idle table because the run had no `TELEMETRY` sink (the windows above are read from the `[FRAME-PROBE]` lines). **The Galaxy A16 row, same APK and scenario (2026-09-25, serial `R5CY8122K7D`, three boots, then an A/B against `r3d13.apk`, four boots alternating with 60 s between; logs local `docs/measurements/device_2026-09-25_galaxy_end8_*.log`):** `[RNG] seeded` -> scenario **7.1-7.4 s vs 7.9 / 8.0** at R3D-13; PSS (before the first mark) **1 278-1 315 MB vs 1 299 / 1 329**, native heap alloc 641-649 MB (vs 681 / 664); detonations (mean / worst / COMMIT ms, first and second grenade): **new 29.8-35.6 / 30.4-34.7, worst 131-157 / 112-139, COMMIT 74-127 / 84-110 vs R3D-13 33.1 / 36.7 and 35.8 / 32.0 mean, worst 239 / 364 and 154 / 307, COMMIT 98 / 110 and 89 / 119**; the shot after the two grenades **`TAIL TOTAL` 211-219 ms** (the same as the restored pre-cook, 218.9-273.8; `r3d13.apk` in the same A/B reads 416 / 420). **Idle is NOT comparable across boots and the cause is the handset, not the build:** the new APK read 25-28 ms/frame in one boot (level with R3D-13's 25.2-25.6; a cool window read 23.1-25.4 at zoom 1.0 against `r3d13.apk`'s 26.1-26.9) and 40-47 in others, and **`r3d13.apk` read 40-44 ms in its own first boot of the A/B**: the phone was charging over USB at ~1.7 A and its battery temperature rose from 23 to 30 C across the session (thermal status 0 throughout). Same lesson as the "first boot after an install", generalised: on the Galaxy a run is only comparable at the same thermal state, so the alternating A/B (not a single row against an old table) is the instrument. **Verdict: no regression on either handset; R3D-END's device gate is closed.**
...
> **RESTORED afterwards (Director: "Religa o pre-cook na 3D", `2d88472e`); final A/B, HEAD APK vs the fix, alternating installs, three pairs: 663.0 -> 418.0, 755.9 -> 220.6 and 418.8 -> 223.3 ms (the first two pairs are noisy, cause not established; the HEAD rows read 419 in the third pair and in every earlier control); pre-cook 383-406 ms in the aim window (776 in the first pair).** Not done: the same A/B on the Moto (not attached); the whole tail is still R3D-LIGHT's. Logs local: `docs/measurements/device_2026-09-24_galaxy_r3d13_*.log` (git-ignored).
