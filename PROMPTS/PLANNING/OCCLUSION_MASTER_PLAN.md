...
| **O3″** | **Occlusion is decided per SLICE — the actual wall-panel unit `SliceGenerator`/`EdgeRegistry` already builds for rendering — using the slice's own real voxel positions, plus a depth cap.** Director's diagrams, 2026-07-14, were explicit: only the wall panel(s) *actually* between the agent and the camera vanish — a wide wall's other slices, and a tall structure's other faces, stay solid; a distant, unrelated wall must not qualify just because it happens to line up on screen. `OcclusionSet.compute_occluded_slices()` reads `room._edge_registry.all_slices()` (the SAME registry instance `VoxelRenderer.render()` just placed on screen, published by `RoomBuilder.build_from_layout()` — not re-derived). For each `Slice` (one face, one gameplay cell, real `Voxel` list with real `grid_pos`/`level`): (1) camera-side, using the slice's own footprint centre; (2) **depth cap** (`max_depth_voxels`, 48) — the corridor test alone is unbounded in depth, and orthographic isometric projection never converges distant things toward the agent's screen column, so without this a far map-boundary wall that happened to line up qualified exactly as well as one right next to him; (3) screen-space corridor overlap against the agent's own silhouette (`silhouette_half_width_px`, widened to 48 after a live test read the first pass as too narrow) — using the SLICE's real half-width from its own corner voxels, not a generic cell-diamond guess. A qualifying slice contributes whole; a slice IS the atomic unit, so there is nothing left to clip. Verified 2026-07-14 against real four-view captures across three iterations (first the corridor-leak fix, then the depth-cap fix) — metal/concrete/stone stay untouched, only the genuinely-in-front panel(s) ghost, and a distant wall segment along the same screen column no longer qualifies. | ✅ Ratified |
...
| **O6′** | **Binary flat-fill + wireframe silhouette outline — replaces O6 (Director decision, 2026-07-13, reference: Phoenix Point/XCOM-style occlusion).** No gradient: every occluded cell gets ONE flat, low alpha (`VoxelRenderer.HIDDEN_FILL_ALPHA`, currently 0.12) via a single ghost alternative (`HIDDEN_ALT_ID`) — "just enough to remember a wall is there." Still zero per-fragment cost — the outline is drawn with `draw_line()`, not a shader — so O6's D12 exemption carries forward unchanged. **Also ratified same session:** no silhouette stroke on the agent (O7 revised below) — full-hide already makes him fully visible, nothing to stroke. **Wireframe mechanism superseded by O3″/OCC-07, 2026-07-14** — see that row: `OcclusionWireframeOverlay` now draws one rectangle per occluded *Slice* (its own real shape — two real corner voxels, real top/bottom levels), not a per-gameplay-cell box with neighbour-suppression logic. The per-cell approach is retired, not carried forward: a Slice needs no suppression, since it IS already the atomic, non-overlapping unit. | ✅ Ratified |
