...
occlusion, and a modular per-unit 3D-box wireframe with a real-voxel dotted
style and glass fill (O3‴/O10–O19, Wave 3.14, closed 2026-07-14). Part 3 (agent-on-top) closed; the silhouette-stroke half is
...
yet — `main` is ahead at VERSION 0.9.15+ with Waves 1 through 3.14. Director's
call on when to tag.
...
1. **Destruction-aware wireframe units.** The wireframe (O13/O14/O15, §6 Wave
   3.12) traces each occluded edge/junction-column's real box as if it were
   intact. The Director's real ideal: once `DESTRUCTION_MASTER_PLAN`'s
   `Slab`/voxel-destruction exists, a unit's own shape should reflect actual
   holes in it — the remaining shell's true silhouette, craters and all — not
   a clean box. Explicitly deferred by the Director 2026-07-14: "isso é luxo,
   ...
   pass are in — O13–O15's per-unit box design is already proven on intact
   geometry.
