...
occlusion, and a modular per-unit wireframe (O3‴/O10/O11/O12/O13, Wave 3.10,
closed 2026-07-14). Part 3 (agent-on-top) closed; the silhouette-stroke half is
...
yet — `main` is ahead at VERSION 0.9.11+ with Waves 1 through 3.10. Director's
call on when to tag.
...
1. **Destruction-aware wireframe units.** The wireframe (O13, §6 Wave 3.10)
   traces each occluded edge/junction-column as if it were intact. The
   ...
   Revisit once both `Slab` and a real destruction pass are in — O13's own
   per-unit design is already proven on intact geometry.
