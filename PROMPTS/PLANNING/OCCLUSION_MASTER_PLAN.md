...
three times over on real Director feedback — current mechanism is the per-EDGE,
graph-walk model (O3‴/O3⁗, Wave 3.7, closed 2026-07-14). Part 3 (agent-on-top)
closed; the silhouette-stroke half is superseded (O7). Part 4 (interior cutaway)
still blocked on `DESTRUCTION_MASTER_PLAN`'s `Slab`. **Runs BEFORE
...
yet — `main` is ahead at VERSION 0.9.7+ with Waves 1 through 3.7. Director's call
on when to tag.
...
| **O3⁗** | **A vertical reveal cutoff (OCC-09) clips the BOTTOM of an occluded edge's tower** — both the ghost fill and the wireframe — **based on how far below the agent's own screen-ground position that tower's lower voxels sit.** Director's insight: an occluding edge is by definition at greater depth than the agent (that's what triggers it), and in isometric projection greater depth alone pushes a tower's own ground level further down the screen than his, independent of real height — a modest single-storey wall sitting some distance away can have its base sit well below his feet purely from that depth offset. Past `vertical_reveal_px` (320px, a live-tunable estimate — the Director offered two equivalent framings, "2 more GU diagonally" and "2 more storeys," that don't reduce to an exact shared pixel value, so this is dialed, not derived) below the agent's own ground screen-Y, a tower's lower levels stay visible. **Real bug found and fixed during this same implementation, not left in**: the first pass computed the clip correctly but only filtered which voxels *contributed* to `OcclusionSet`'s per-column dict — since that dict is keyed by (x,y) only, with no level, any surviving voxel at any level caused `VoxelRenderer.apply_occlusion()` to ghost the ENTIRE column regardless, because it walks every level of a listed column with no way to know which ones were meant to be excluded. Only the wireframe (which reads a real min/max level pair, not a flat dict) was actually clipping — the fill kept covering the whole tower, just imperceptibly at the new low alpha. Fixed by carrying `min_level` inside the per-column dict entry (`{"ring": int, "min_level": int}`) and having `apply_occlusion()` start its level loop there instead of at 0. Confirmed via a live diagnostic dump (not just the visual) that `clip_min_level` varies correctly per edge by real depth before shipping. | ✅ Ratified |
