...
graph-walk model with a fixed always-visible base band (O3‴/O10, Wave 3.8, closed
...
yet — `main` is ahead at VERSION 0.9.9+ with Waves 1 through 3.8. Director's call
on when to tag.
...
1. **Destruction-aware wireframe contour.** The wireframe (O12, once rebuilt —
   see §6 Wave 3.8) traces the outline of the whole occluded object as if it
   were intact. The Director's real ideal: once `DESTRUCTION_MASTER_PLAN`'s
   `Slab`/voxel-destruction exists, the contour should trace around actual
   holes in the shape — the remaining shell's true silhouette, craters and all
   — not a clean box. Explicitly deferred by the Director 2026-07-14: "isso é
   luxo, numa situação de processamento sobrando" (a luxury for when there's
   ...
   are in and this plan's own contour-tracing mechanism (O12) is proven on
   intact geometry first.
