...
**2026-09-26 (later) update (v1.30) — R3D-LIGHT STEP 1 BUILT: INCREMENTAL OCCUPANCY (Director: "Vamos com R3D-Light, usando o criterio de 100ms").**
- **The criterion in force:** no blast or shot frame above **100 ms on the Moto**. No handset was attached (no `adb` on the PATH), so what follows is the desktop, in the v1.29 recipe; the Moto number is still owed.
- **Built.** `VoxelStore` keeps a live occupancy (`occupancy_live()`) and a journal of the cells a visibility write touched (`mirror()`, `_recompute_cell()`); a sync applies the journal to the dictionary and reports the cells whose membership changed. `VoxelLightField.build(..., live_changes)` derives its stale set from those cells when it already holds that very dictionary (`is_same`), and falls back to the full diff otherwise (first build, the cook's predicted occupancy). The two repaint sites in `room.gd` use it; the cook's `_predicted_occupancy()` uses `occupancy_live_after()` (a per-level copy less the gone claims: a snapshot on purpose, so a repaint between the cook and its commit cannot move the cook field's world).
- **Measured (desktop, PLAYGROUND, the v1.29 recipe):** the shot's occupancy **66.0 -> 1.7 ms** (the repaint's total 103.6 -> 61.4 in one sample; `field.build` and `apply` read higher in that sample, 12.5 -> 23.8 and 25.1 -> 35.9, one boot each: noise not separated); the cook's predicted occupancy **39 -> 15 ms** per grenade. The COMMIT frame (34-40 ms) and the CONSEQUENCE frame (44-55 ms, of which the cook field's bucket derive is 35 ms) are unchanged: **they are the next candidates**, and the ~7x Moto factor says they need to fall below ~15 and ~30 ms here to meet 100 ms there.
- **Gates.** `INFILTRAITOR_OCC_LIVE_PROBE=1` (new, `Room._occ_live_probe()`): 0 of ~215 000 cells differ from the full walk after every sync through two grenades, a shot and two rotations; `LIGHT_COOK_GATE` 0 and 0 PASS; `voxel_store_selftest` TEST 8 (the journal, the shared-cell case, `occupancy_live_after()` against `occupancy_dict_after()`, and a sabotage: a write that skips the journal is caught); `verify.py` (smoke) PASSED.

- **Moto g04s, step 1 measured (release APK `export/light1.apk`, `ad5ba534`, END-8 recipe, two boots, logs `docs/measurements/device_2026-09-26_moto_light1_{1,2}.log`, git-ignored):** the shot after two grenades **`TAIL TOTAL` 249 / 250 ms vs 526 / 527 (END-8)**. The blast frames are NOT below 100 ms yet: first grenade / second grenade, boot 1 then boot 2: COMMIT **256 / 231, 249 / 234** (END-8: 234-241), CONSEQUENCE **245 / 178, 229 / 143** (155-236), LIGHT beat's first frame **168 / 168, 155 / 171**, SOOT FADE first frame 67-115, cook pump max 131-157, `derive` of the cook field's apply **87-90 ms**; detonation mean 30.0-32.2 ms. **Still over 100 ms per blast: COMMIT, CONSEQUENCE, LIGHT (derive), the pump's worst frames.** Next: decompose each of those frames with clocks before choosing (a time-sliced derive must respect the ramp's rule that nothing is presented between the apply and the rewind, `play_consequence_light`).

**2026-09-26 update (v1.29) — THE POST-CLOSE CLEANUP IS DONE, R3D-SURFACES IS PLANNED, R3D-LIGHT IS PLANNED FOR THE NEXT SESSION, AND THE DIRECTOR CLOSES R3D AT R3D-LIGHT ("Menos iniciativa e mais acabativa").**
Commits `2bb5cbb1`..`200986da` (the cleanup, recorded in the v1.28 block below) and `eb7e32c8` (R3D-SURFACES). Session record: `PROMPTS/RESUMO_SESSAO_2026-09-26_CLEANUP_E_R3D_LIGHT_PLAN.md`.
- **The closing rule (Director).** R3D closes when **R3D-LIGHT** meets its budget. R3D-ACTORS, R3D-PROPS, R3D-WORLD, R3D-ROT, R3D-LOOK, R3D-SURFACES, R3D-CLAIMS and R3D-BUFFER are follow-on tracks with their own plans, not R3D debt: the rotation works today (a full re-layout), the actors and props work as billboards, and the only goal of the track still unmet is the hitch frames.
- **R3D-LIGHT, measured on the desktop this session (PLAYGROUND, `RNG_SEED=1`, grenades `25,2;37,2`, `detonate 0; detonate 1; shoot 0`, `EVENT_FRAMES=1`, `FRAME_PROBE=1`, `REPAINT_PROFILE=1`; log `docs/measurements/desktop_2026-09-26_r3d_light_hitch.log`, git-ignored).** The shot's tail is **112.6 ms, of which the light repaint is 103.9: occupancy 66.0 · `field.build` 12.5 · `apply` 25.1** (the scoped repaint over an 84-GU stale set). Per grenade: the cook's pump ends after 18 frames (max 50.7 ms), COMMIT frame 32 ms (`apply` 33.6, then 15.2 ms), CONSEQUENCE frame 62.5 / 43.8 ms, and the **LIGHT beat's first frame 68.7 / 68.2 ms** (`[CONSEQUENCE] light ramp ... derive 35.5 ms (cook field)`). The Moto ran 3-7x slower than this desktop on the same frames in the R3D-13 tables (COMMIT 32 -> 235 ms, the shot 112 -> 526), so these are proportions, not a prediction.
- **The lever.** `VoxelStore.occupancy_dict()` walks all ~216 000 claims and builds a new `Dictionary` (one `Vector2i` key per occupied cell) at EVERY light repaint, and `VoxelLightField._stale_cells()` then diffs it against the previous one, to learn that a few hundred cells changed; the store already knows which, because every visibility write goes through it. The walk is in the shot's repaint, in the cook's atomic LIGHT phase (`occupancy_dict_after()`, ~190 ms on the Moto) and in the pre-cook.
- **Step 1 (next session): incremental occupancy.** The store keeps a journal of the cells whose visibility changed; the light field applies it to a persistent occupancy dictionary and derives the stale set from the changed cells only (no map walk, no full-dictionary diff); the pre-cook's predicted world is a speculative delta the next real sync reverts first. **Gates:** an equivalence probe (incremental dictionary vs the full walk after every sync, 0 differences through a blast, a shot, a rotation and a restore), the existing `LIGHT_COOK_GATE` (0 and 0 in the last rounds), and a `voxel_store_selftest` case for the journal with a sabotage that must fail. **Then re-measure before designing more:** the 35 ms derive of the LIGHT frame and the COMMIT frame are the next candidates, and this plan's lesson is that the light's costs have been guessed wrong before.
- **The criterion to close (proposed, awaiting the Director's confirmation):** no blast or shot frame above **100 ms on the Moto**, and the mean still at 30 fps idle / 24 fps under load. The final number is a Moto (and Galaxy) measurement: it needs the phone connected (`tools/persistent/device_run.py`; `adb` was not on the PATH in this session and no device was attached).
- **Also decided this session (Director):** the atoms cluster is retired and `EarthVariantSelector` kept; `occlusion_overlay.gd` stays (LIGHT_VISION suite); FLOOR-DEPTH-01 is resolved (its constant removed); **the weapon bench stays untouched until new weapons exist**; R3D-SURFACES is designed (below, After R3D-END) and not started.
...
- **Post-close cleanup (2026-09-25, Director: "vamos limpar o codigo morto primeiro"):** the dead code the END left on purpose is gone: `VoxelBoard._glass_neighbour`, `FacadeSampler.get_window_origin_*` (and the two private `_window_origin_*` they alone reached), `GlassCrack.has_craze_art`, `GlassMaterials.fracture_texture_id`, `OcclusionSet._is_exposed` (two comments repointed; the wireframe builder's header, stranded above it, moved back onto it), `theme_applier.gd`, and the scratch `test_keys.gd` at the repo root; and the `tile_size` of `shadow_boundary_overlay` (never read), `shadow_overlay` and `temporal_overlay` (written by `VisionController`, never read: declaration and write went together). The session summary called these "unused fallbacks"; that was half right: `VisionController` assigns `tile_size` on seven overlays and the other four READ it (`light_overlay`'s lamp radius among them), so only the stale "fallback only when floor_layer is unset" comments went there. Each had zero readers repo-wide (code, scenes, tools); last present in `6bb89cb4`. `verify.py` smoke: 51 selftests clean, PLAYGROUND and GLASS boot and rotate. **Batch 2 (same day):** `FacadeSampler.sample()` and its two `_mirror_*` helpers (the class is now the FNV-1a owner only), 26 preload constants nothing in their own file reads (21 files), and the three `r3d4a_*` shaders of the deleted actor spike. **Still open:** the five "dev-tool paths" (never enumerated anywhere); `occlusion_overlay.gd` (kept, Director 2026-09-26: part of the LIGHT_VISION suite, OCC-21m); the 34 declared-but-never-referenced symbols kept after inspection: 24 design stubs (`TYPE_*` light types, `NOISE_*`, `POSTURE_*`, `STEALTH_*`, the `SaveState` file API) and 10 that are documentation or an unbuilt ruling (`THROW_PROFILE_ENV` documents a live flag, `STROBE_SEQUENCE`, `is_combustible` (its callers re-derive `> 0.0` inline), `SEG_SIZE`, `vision_mode`, `_reset_room_state`, `clear_head_yaw`, `FRAME_SWAP_HZ` and `SHADOW_ELEVATION_DEG` (config of a bake that lives until R3D-PROPS), and `DEEP_FLOOR_CRATER_FACTOR` was removed after the Director said FLOOR-DEPTH-01 is resolved (2026-09-26)); `WeaponBenchController` (~450 lines wired in `room.gd`). **Director, 2026-09-26: the bench STAYS as it is until new weapons arrive** (variations of the main ones, with other specs and art; the materials are nearly final); "cross that bridge when we get to it". Note for that day: the triage removed its `add_weapon()`, `TEST_ZONE_WEAPON_ROWS` and `TEST_ZONE_WALL_GU_X` (and their design notes: the bench is a weapon x material matrix, rows by range, shotgun row y=6 because `flood_gu_cone()` caps at ring 4) — recover them from `f4ddd52c`; a simplification was proposed (rows and columns from `WeaponRegistry` / the map's materials, firing through the agent's shot path) and not started. Also deferred: whether organic ground keeps a photographic floor (one `facade_<id>` path with a per-material `photographic` flag was the recommendation); the `damage_materials` map section (still compiled by `MapCompiler` and forwarded by `FileMapSource`, nothing reads it since the `DamageVariantBaker` went).
  **Atoms retired (2026-09-26, Director: "Aposentar, manter o EarthVariantSelector"):** 73 files (the 17 `voxel_*.png` atoms, their `.import` files and every `halves/` folder; none was in git) moved to `ARCHIVE/voxel_atoms_2d/` with their relative paths (restore with `mv`); `bake_policy.gd`, `high_wall.gd` (`HighWallGroup`) and `tile_anatomy_audit.gd` git-removed; `EarthVariantSelector` kept, without its PNG path helper, as the B4 / R3D-LOOK reference. Tests that lost their subject: `material_reform_selftest` 12 -> 8 assertions (the wall == floor policy check and the three canonical-atom checks; the `has_facade` flags of `concrete`, `grass`, `earth` and earth's `base_color` stay) and `earth_variant_selftest` 6 -> 5 (the PNG size check). **Found on the way, for R3D-LOOK:** `BakePolicy.texture_for_material()` was the only statement in code of D34's rule that organic ground (`has_facade == false`) keeps a photographic `slab_<id>` floor; the 3D board has no such path (a `has_facade == false` material draws its flat base colour), so that rule is prose only now (`ART_SPECIFICATIONS`, `MaterialRegistry`). `generate_voxel.py` is a mixed tool: `main()` would still write the atoms and halves if run (a note in its header says so); only `build_decal_family()` feeds anything live.
  **Triage (2026-09-26, Director: "apagar os mortos, manter os de design"):** 55 dead declarations removed with their doc comments (49 from the list plus 6 the removals orphaned: `CELL_KINDS`, `OUTLINE_DISABLED`, `WEAPON_GRADE_*`, `Voxel.face_atlas_rect`, a preload), among them 14 the R3D stages themselves orphaned (`project_voxel`, `get_voxel`, `visible_of`, `PLAYED_KINDS`, `EXPOSED_FLOOR_SOOT_RING`, `TINT_SLOTS`…). The removal tool's first dry run took ten live `@export var` lines with one deletion, which is why every removal is reviewed in a log before it is applied. `VOXEL_STOREY_HEIGHT_PX` was restored: two comments of `iso_projection.gd` derive the vertical axis from it.
  **Comment audit (same day):** 48 comments that named something deleted (functions, shaders, tests, flags, `VoxelBoard.apply_occlusion()`, `glass_pane.gdshader`, `INFILTRAITOR_PRESENTER`) were rewritten against the current code; the P-WARM and W-PRECOOK headers were condensed to what the code does now (`_warm_prediction()` only marks the job warmed; the shot pre-cook warms the light field only); `check_facade.py`'s comments stopped pointing at `bake_compositor.gd`; and `project_lint_validator.gd` lost an exception for a test file that no longer exists. **Left on purpose:** comments that record the past and say so (dated measurements of the 2D TileSet mint cost, the CRACK-03/04 design narratives), and every comment inside the atoms cluster, which goes with it.
...
- **R3D-SURFACES — photographic horizontal surfaces (PLANNED 2026-09-26, Director; not started, not a blocker).** Two mechanisms on purpose: the facade (grayscale x `base_color`, walls and everything procedural) and a photographic surface (colour, opaque, horizontal faces only: floors, roofs). A material declares its surface per role (wall, floor, roof) instead of one `has_facade` flag; **the wall stays a facade always** (D34 unified wall and floor to kill the seam at the wall base). Design, agreed in debate:
  1. **Base: one large plane per surface, sampled in world space.** 1024 x 1024 texels = 64 x 64 voxels = 8 x 8 GU at the pinned `TEX_AUTHORING_N = 16` (the old isotropic `slab_<id>` spec). REPEAT, never mirror (mirroring is fine on grey pattern and ugly on a photo); the art only has to tile at its own two borders. Opaque, no Godot blend mode, its own shader for top faces (no branch in the shared shader: the untaken-branch cost on Mali). **Rejected: a tile per GU with hashed variants** — neighbouring variants do not match at the borders (Wang-tile problem), and a world-space plane is continuous by construction; texel density per area is the same, only the unique area differs.
  2. **Macro modulation:** one small extra map (~256^2) with a period coprime to the plane's (~61 GU) varying brightness / hue, so the 8 GU repeat is not perceptible on a 44 x 22 GU map. One extra sample.
  3. **Patches are decals, not materials:** mud, foliage, puddles, dirt (things that fit in a GU and have their own shape) ride the existing decal path (`floor_decal3d`, variants per GU). Base = the material; patch = a layer. Roofs use the same base, anchored per roof component (`Slab.texture_anchor`, ART-01) so the tile rows do not break between structures.
  4. **Transitions between two bases: a hard edge at the cell for now.** A soft blend would be a per-cell mix plane (like the soot plane) sampling two surfaces; later, if wanted.
  **Not measured, to measure first:** the macro sample on the Moto (already GPU-bound), the ASTC size of a 1024^2 plane (estimated 1-2 MB), and how organic ground draws today (a `has_facade == false` material is a flat base colour on the 3D board). New gate needed: `check_surface.py` (size, no alpha, border difference) beside `check_facade.py`, which rejects colour. Sources are CC0 only (D57). **First prototype when taken up:** a grass plane + macro map + one leaf-patch decal, measured on the Moto, before any expansion.
...
- **v1.29, 2026-09-26.** The post-close cleanup (about 1 550 net lines of dead code and stale comments, the atoms cluster retired), R3D-SURFACES planned, the closing rule (R3D closes at R3D-LIGHT) and R3D-LIGHT's measured baseline and step 1 (incremental occupancy).
