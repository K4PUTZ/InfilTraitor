...
| **F2** | **A `BackBufferCopy` cannot see the cells of the layer it precedes — so the merge traded one bug for another.** §19.2 claimed the snapshot would contain *"the opaque cells of level N that sit behind it"* "because it y-sorts inside its own layer". A `BackBufferCopy` copies the framebuffer at **its own** point in the draw order; it draws before the whole layer, so none of that layer's cells are in it. With `cover ≈ 1` for a pane body (`glass_pane.gdshader:47` *(deleted at R3D-END; last in `34881f81`)*), an opaque wall **behind** the glass at the same level would have been replaced by tinted background. It fixes the front wall and breaks the back wall. |
...
`COPY_MODE_VIEWPORT` (`voxel_renderer.gd:6254` *(deleted at R3D-END; last in `34881f81`)*).
