...
| `render()`, `render_block()`, `render_slab()`, `render_slab_solid()`, `render_fixed_earth_level()`, `render_prop()`, `_render_slice()`, `_render_junction_column()`, `_ensure_layer()` / `_ensure_voxel_layers()` / `_ensure_negative_voxel_layer()`, `_assert_geometry_rendered()` | Nomes de quando o `VoxelRenderer` colocava tiles (renomeados no END-6: agora só registram níveis) | `register_geometry()`, `register_block_levels()`, `register_slab()`, `register_slab_solid()`, `register_fixed_level()`, `register_prop()`, `_register_slice()`, `_register_junction_column()`, `_ensure_level()` / `_ensure_wall_levels()` / `_ensure_floor_level()`, `_assert_geometry_registered()` |
