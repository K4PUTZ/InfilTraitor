...
- **[EXPLOSION_REBUILD_MASTER_PLAN](../PROMPTS/PLANNING/EXPLOSION_REBUILD_MASTER_PLAN.md)** — 🟢 BUILDING, Tasks 0–3 done (2026-08-06/07). Supersedes `DESTRUCTION_MASTER_PLAN` Part 3 for explosions specifically. 4-ring spherical model (D14), **273 real** pre-baked damage atoms shared across floor/wall/ceiling (Task 1b measured, not the projected 207), a 15-wave `set_cell()`-only choreography. Task 2 (E-RING) shipped the ring/falloff calculation surface; Task 3 (E-SOOT) shipped blast-stamped soot, keeping full per-face directional soot rather than the plan's original per-voxel collapse (a stale processing-cost concession, confirmed with the Director). §3.5/D13: per-map declared material scope + cross-session bake cache, because materials are headed toward per-player procedural content, not a fixed catalog. Narrows `WEAPON_MASTER_PLAN`'s firearm-untouched boundary by one step (bullet-mark *painting* only, D12 — see that plan's §3; already live, a zero-code side effect of Task 1b). **Calculation-layer only so far — no live caller** (`TestZoneController.detonate_active()` still disconnected). Resume point: §11 — Task 4 (E-PLAN), the `DetonationPlan` builder, is next.
