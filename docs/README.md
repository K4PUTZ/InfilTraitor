...
- **[EXPLOSION_REBUILD_MASTER_PLAN](../PROMPTS/PLANNING/EXPLOSION_REBUILD_MASTER_PLAN.md)** — 🟢 BUILDING, Tasks 0–5 done (2026-08-05→07) — **Phase A is functionally complete: a grenade detonates and damages voxels for real, on screen** (`Screenshots/history/e_wave_detonation.png`), for the first time since the 2026-08-05 reset. Supersedes `DESTRUCTION_MASTER_PLAN` Part 3 for explosions specifically. 4-ring spherical model (D14), **273 real** pre-baked damage atoms shared across floor/wall/ceiling (Task 1b measured, not the projected 207), a real 15-wave `set_cell()`-only choreography (`DetonationChoreographer`, independent `SceneTreeTimer` per wave, 40 ms cadence) built from a fully pre-resolved `DetonationPlan` (`DetonationPlanBuilder`, proven never to touch the live TileMapLayer before its own wave fires). Task 2 (E-RING) shipped the ring/falloff calculation surface; Task 3 (E-SOOT) shipped blast-stamped soot, keeping full per-face directional soot rather than the plan's original per-voxel collapse (a stale processing-cost concession, confirmed with the Director); Task 4 (E-PLAN) folded all resolution/exposure-fallback/lighting into one pre-computed plan; Task 5 (E-WAVE) plays it back and reconnects `TestZoneController.detonate_active()`. §3.5/D13: per-map declared material scope + cross-session bake cache, because materials are headed toward per-player procedural content, not a fixed catalog. Narrows `WEAPON_MASTER_PLAN`'s firearm-untouched boundary by one step (bullet-mark *painting* only, D12 — see that plan's §3; already live, a zero-code side effect of Task 1b) — firearms confirmed unaffected by every task, including a real fire capture after Task 5. **2026-08-08:** found and fixed a silent GPU-upload-flush bug (`DamageCompositeCache.store()` defers the texture upload to `flush_dirty_pages()`; `DetonationChoreographer` — the only place a real `DetonationPlan` reaches `set_cell()` — never called it) affecting every real detonation's dented/cracked/soot marks; re-ran the Post-Task-5 "fuligem quebradiça" A/B test clean afterward and **reversed its conclusion — the blast's own soot stamp IS the cause**, not pre-existing decal art (see the plan's Post-Post-Task-5 note). Also surfaced that floor (SLAB) and wall (SLICE) textures were genuinely different art/render pipelines (dimensions, color rules, resolver validation), which is why metal/stone/wood floor damage never baked. **2026-08-08, later — that seam is now formalized and closed: D34 + D35.** The SLAB/SLICE split is gone for structural materials (a floor is a roof at the base of the scene: one grayscale `facade_<id>`, one MULTIPLY tint, roof and floor sharing one page; `slab_<id>` kept only for organic ground), the horizontal projection is isotropic everywhere and reaches it by **mirrored vertical repeat rather than a stretch** — which also fixed a latent roof bug where cell rows past ~36 had no texels — floor dents wear their own material's decal art, and `earth` became a buildable material (walls, blocks, roofs). Commits `8dd926e`, `22b24be`, `9cd37ae`, `87fa023`, `6ec2566`. **Open:** Phase B (targeting UI, throw arc, explosion frames) not started; **Task 6's tuning pass is now the next concrete action** (including the soot-stamp rendering mechanism, still untraced), plus the GPU-flush safeguard — resume point §11.
