...
- **[ACTOR_MASTER_PLAN](../PROMPTS/PLANNING/ACTOR_MASTER_PLAN.md)** — 🟡 v1.2: twin (showcase-only) split from a separate runtime "simplification" system. Objects track (shotgun, Showcase screen) open now; living-beings track (character twin/poses/damage) deferred.
