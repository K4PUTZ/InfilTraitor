...
## Where the project stands — 2026-09-26 (newest; the 2026-09-25, 2026-09-24, 2026-09-21 and 2026-08-30 sections below are kept as history)

**Version 0.9.107 · R3D-END is closed, the post-close cleanup is done, and the Director closes R3D at R3D-LIGHT (planned for the next session).** Record: the v1.29 block at the top of [`RENDER3D_MASTER_PLAN`](../../PROMPTS/PLANNING/RENDER3D_MASTER_PLAN.md) and `PROMPTS/RESUMO_SESSAO_2026-09-26_CLEANUP_E_R3D_LIGHT_PLAN.md`.
- **The cleanup (9 commits, `2bb5cbb1`..`200986da`, about 1 550 net lines):** the dead code the END left on purpose, 26 unused preload constants, 55 declared-but-never-referenced symbols (34 more kept after inspection: design stubs and documentation of live flags), 48 comments that named something deleted, and the 2D voxel-atom cluster (`BakePolicy`, `HighWallGroup`, `tile_anatomy_audit`, the atoms and `halves/` moved to `ARCHIVE/voxel_atoms_2d/`, two selftests lost the assertions whose subject went: 12 -> 8 and 6 -> 5). Every commit passed `verify.py` smoke (51 selftests, PLAYGROUND and GLASS boot and rotation).
...
- **Left, stated:** dead code that predates the END (swept 2026-09-25: `_glass_neighbour`, `FacadeSampler.get_window_origin_*`, `GlassCrack.has_craze_art`, `GlassMaterials.fracture_texture_id`, `OcclusionSet._is_exposed`, `theme_applier.gd`; the five dev-tool paths were never enumerated, so they are still open); the 17 `voxel_*.png` atoms and `halves/` folders that no runtime code reads (`ASSETS/materials` is not in git); the "zero warnings" rule cannot be checked from the command line; no selftest pins the FNV-1a output values.
...
- **R3D-END's entry gate read PASS (its last run was END-0; it retired at END-1):** `independence_gate.py` emptied the hidden 2D board after every step of a real run and requires the same world and the same picture (0 cells held at 19 steps, 19 dumps identical, 0 px above noise), with a control that must still fail.
...
- **Suite:** 64 selftests clean, none pinned to 2D; gates: `board_probe.py gate|shadow|roundtrip`, `pixel_gate.py` (`--against` a stored set), `shot_3d_gate.py`, `mirror_gate.py`, `occ_canonical_gate.py`, `ground_gate.py` (recorded digests).
- **What is next:** R3D-END END-2 (the glass tiles and `GlassCrackSprite`), END-3 (the 2D cutaway), END-4 (tile placement and the atlases), END-5 (`floor_layer`), END-6 (instruments, tools, the class rename — the Director picks the name), END-7 (canon), END-8 (the device matrix). The class is stripped in place and renamed at the end (Director, 2026-09-24).
...
1. **GLASS — ✅ PHYSICS DONE. Calibrated 2026-09-10; one RENDER task designed, not built.**
   ...
   G-VARIANT family, G-ART (procedural fracture sheets), the CRACK as a sprite over
   the pane (CRACK-02, after CRACK-01 in the voxel shader was rejected 3×), the
   blast CRAZE family (G-D35), G6 shards on the floor, and §18's shard rain end to
   end: **G4-1** the 5-shape family · **G4-2/3** remnants stuck in the frame
   (real-map confirmed) · **G6b-1** the field (one texture, one draw call) ·
   **G6b-2** the fall (aged in FRAMES) · **G4-4** the scatter table + the shockwave
   impulse (the pile went from a ~48-cell line to a 284-cell band) · **G-D45** a
   remnant whose frame is destroyed falls with it (`glass_reap_demo` PASS on the
   real map) · **G6b-3** the dust puff. The §8.1/§8.2 CRACKED-tier blocker was
   resolved by Director ruling 2026-09-02 (G-D3 removed the need; the crack is a
   sprite, not a decal family). **Left in M4:** `plastic` (screen backing),
   skylights (G-D16c/d — `lift` is authored but has no geometry to run on), and
   S-4's four-class fracture art (order written, art deliberately not made).
   Three findings that belong to systems OUTSIDE glass are still open: **`NoiseSystem.emit()`
   has zero call sites**; vision has no material, so **glass blocks sight like
   concrete** (a balance number, not glass art).
...
1. ✅ **GLASS §8.1 / §8.2 — RESOLVED 2026-09-02 by Director ruling.** G-D3 removed
   the need for a CRACKED `INTACT` return; CRACK-02 makes the crack a SPRITE over
...
3. **Should a breached OPAQUE wall open a passage for MOVEMENT?** The machinery is
   ...
   *"3 granadas no mesmo lugar com concreto"* ruling), and as of 2026-09-07 a
   breached wall opens the grenade's blast flood. But `build_movement_edge_set()`
...
6. **AI-02 resume timing** — open since 2026-07-26 and never re-raised.
...
- RESUMO_SESSAO_2026-08-27_28_THE_CHOREOGRAPHER_IS_THE_VILLAIN.md
...
- RESUMO_SESSAO_2026-09-06_G4_4_SCATTER.md
...
- RESUMO_SESSAO_2026-09-10_OPTION_A_SHIPPED.md
...
> non-local pane break. ✅ **DONE 2026-09-06** — `GLASS_MASTER_PLAN` §18 closed:
> the grenade shatters the whole pane, the shards scatter into a band biased by
> the shockwave, land, puff dust, and leave a pile; a remnant whose frame the
> blast also broke falls with it.
>
