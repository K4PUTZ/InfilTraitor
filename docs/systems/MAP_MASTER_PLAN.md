...
| `roofs` | Array | — | `[{gu, size, storeys, material, kind}, ...]` free-standing roofs (R3D-7, 2026-09-20): a rectangle of ceiling slabs sitting on top of `storeys` storeys with NO walls beneath (a roof over a room's floor, a floating canopy); `kind` is `"flat"` (the seam for pointed/diagonal roofs). Not derived from `blocks`, which still carry their own roof. Compiled to `roof_instances`, rotated by `PerspectiveMapper`, generated by `RoomBuilder` through the block-roof code. See `MAPFILE_REFERENCE.md` |
