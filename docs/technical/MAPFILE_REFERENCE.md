...
| `panels` | **HALF-THICKNESS elements** (M3-2b, 2026-08-21): `{gu, face, material, storeys, start_storey}` — a window pane, a curtain, a cardboard partition. ONE storey-face on ONE of the two GUs an edge separates, where a normal wall is two (D16). ⚠️ **`gu` IS the side**, as an ABSOLUTE cell, and never a boolean: `Edge._init()` swaps `gu_a`/`gu_b` when the face points NW or NE, so `side_a: true` would silently mean different things depending on which way the author drew the wall. `Edge.set_occupied_gu()` resolves the cell AFTER canonicalisation. A panel whose edge already exists fails loudly rather than merging |
| `floor_zones` | author-declared floor-bake ground material rects: `{gu, size, material}` (same shape as `blocks`; FLOOR-BAKE-01, see BAKE_SYSTEM_REFERENCE.md FLOOR-ZONE-BAKE). **D34:** a `has_facade` material here bakes from its own `facade_<material>` — the same source its walls and roofs use — and shares their page; only `has_facade: false` materials (organic ground) use `slab_<material>`. **`"earth"` is not declarable here**: it doubles as the sentinel for "this GU has no zone", so a rect declaring it is ignored and warns (D35 scoped earth to walls/blocks/roofs) |
