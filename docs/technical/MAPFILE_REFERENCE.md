...
| `walls` | edge runs as `a→b` spans: `{a, b, material, storeys, facade}`; `"facade": null` = the material's default facade, `facade_<material>` (derived where the facade is loaded; `BakePolicy` did it until 2026-09-26) (M6 prefix canon) |
...
| `roofs` | **free-standing roofs, an entity of their own** (R3D-7, 2026-09-20): `{gu, size, storeys, material, kind}` — a rectangle of ceiling slabs sitting on top of `storeys` storeys of height, **with no walls beneath**: a roof over a room's floor, or a floating canopy. A solid block (`blocks`) still carries its own roof; this is the same roof without the block, generated by the same code (`room_builder`, two destructible levels, same border rules, same texture anchoring), and rotated with the view like a block (`PerspectiveMapper`, ROOF-BAKE-02a). `kind` is `"flat"` and nothing else is implemented — any other value is a loud `push_error` and the roof is skipped; it is the seam for the pointed and diagonal roofs planned later. Builder + walls are the two mechanisms a scenario builder is meant to use together to make a room. **Occlusion:** a roof an origin stands UNDER opens by adjacency of its slabs (`OcclusionSet.ROOF_REACH` = 6, the last `ROOF_FADE` = 2 fading; corner-touching counts), see `RENDER3D_MASTER_PLAN` R3D-7 (2026-09-21: any other `kind` is skipped with a `push_error`, builds no slab, and the rest of the map still builds: pinned by `roof_entity_selftest`.) |
