...
##
## ⛔ G-D35 B-4's PERFORATION WAS BUILT AND ABANDONED (2026-09-05).
##
## The class's destruction axis — *"queremos que alguns decals sejam perfurados
## aleatoriamente em alguns voxels"* — shipped, was photographed on the real pane,
## and the Director ruled against it by looking: *"ficou confuso visualmente, e eu
## acho que é muito trabalho para pouca vantagem ficar furando parcialmente […]
## abandona. Vamos usar o rachado sem furos."*
##
## ⚠️ REMOVED RATHER THAN LEFT AT RATE 0. A dial set to zero is a feature that is
## built, wired, tested and never triggered — the shape of the two this project
## already shipped and had to find later (the noise indicator, the exposure
## labels). The code is in `9e66b409` if it is ever wanted back, and
## `GLASS_MASTER_PLAN` §16.13 keeps what it MEASURED, which is the part worth
## having: FNV-1a cannot be thresholded for a few-percent slice — it selected
## whole columns — and the fix is a murmur3 finalizer at the call site, never a
## change to `_fnv1a_hash` itself (B4 pins it).
##
## Returns Array[Voxel].
