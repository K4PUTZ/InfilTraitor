...
func _init() -> void:
	...
	if not _test_toggle_identical_coords(mock_config):
		all_pass = false
	if not _test_baking_on_with_hit(mock_config):
		all_pass = false
	...
	print("\n" + "=".repeat(60))
	if all_pass:
		print("BAKE-05 SELFTEST: 5 / 5 PASS")
		print("=".repeat(60) + "\n")
		print("✓ SELFTEST PASS")
	else:
...
## Test 4: With baking enabled and baked atlas populated, lookup HITS
func _test_baking_on_with_hit(bake_config) -> bool:
	print("[TEST 4] baking_on_with_hit\n")
	var success = true

	bake_config.set_enabled(true)

	# First, construct the key that will be generated by resolve()
	# We need to match how _make_bake_key() computes it
	var wall = MockWall.new("mat_1", "facade_1")
	var edge = MockEdge.new("test_edge", wall)
	var face = 0  # NE
	var voxel = Vector2i(8, 0)

	# Manually construct what the lookup will construct
	var lookup_ref = BakedTileLookupClass.new()
	var bake_key = lookup_ref._make_bake_key(edge, face, voxel)
	var key_str = lookup_ref._bake_key_to_string(bake_key)

	print("    Generated key: %s" % key_str)

	# Now populate the baked atlas with this exact key
	var baked_atlas = BakeCompositorClass.BakedAtlas.new()
	baked_atlas.lookup[key_str] = {
		"page": 0,
		"atlas_coords": Vector2i(5, 3)
	}
	Engine.set_meta("GLOBAL_BAKED_ATLAS", baked_atlas)

	# Query with a fresh lookup instance
	var lookup = BakedTileLookupClass.new()
	lookup.set_test_config(bake_config)
	var result = lookup.resolve(edge, face, voxel)

	# The key point: with string keys, lookup should HIT
	if result.source_id == "BAKED_ATLAS_0":
		print("    ✓ Lookup HIT: resolved to baked atlas (source: %s)" % result.source_id)
		print("    ✓ Atlas coords: %v" % result.atlas_coords)
	else:
		print("    ✗ Lookup MISS: got %s (expected BAKED_ATLAS_0)" % result.source_id)
		print("    (This would fail with object-identity keys; string keys make it work)")
		success = false

	if success:
		print("  PASS: baking_on_with_hit\n")
	else:
		print("  FAIL: baking_on_with_hit\n")
	return success


## Test 5: Grep audit (verify no residual references exist)
