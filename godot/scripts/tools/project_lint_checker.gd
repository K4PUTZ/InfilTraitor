## PROJECT_LINT_VALIDATOR — Simple GDScript parse checker (no autoload dependencies)
## Walks res://godot/scripts/ and loads every .gd file to check for parse errors
...
func _ready() -> void:
	...
	get_tree().quit(1 if failed_files.size() > 0 else 0)
...
func _check_file(gd_path: String) -> void:
	...
	if result == null:
		...
		parse_errors.append("Parse error: %s" % gd_path)
		# Don't print here; errors are already printed by Godot during load()
	else:
		print("  ✅ %s" % gd_path)
